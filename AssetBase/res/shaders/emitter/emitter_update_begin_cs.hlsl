...
void cs_main(uint3 thread_id : SV_DispatchThreadID)
{
	...
	indirect_buffers.Store3(COMPUTE_IAB_OFFSET_DISPATCHEMIT, uint3(ceil((float)real_emit_count / (float)THREADCOUNT_EMIT), 1, 1));

	// Fill dispatch argument buffer for simulation (ThreadGroupCountX, ThreadGroupCountY, ThreadGroupCountZ):
	indirect_buffers.Store3(COMPUTE_IAB_OFFSET_DISPATCHSIMULATION, uint3(ceil((float)(alive_count_new + real_emit_count) / (float)THREADCOUNT_SIMULATION), 1, 1));
