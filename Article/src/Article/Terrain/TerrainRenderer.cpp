...
void TerrainRenderer::RenderFramebuffer()
{
	...
	for (int i = 0; i < RenderWidth * RenderWidth; i++)
	{
		...
		const TerrainType& terrain0 = Li::Application::Get<ArticleApp>().GetTerrainData().GetTerrainPrototype(chunk.Tiles[0]);
		const TerrainType& terrain1 = Li::Application::Get<ArticleApp>().GetTerrainData().GetTerrainPrototype(chunk.Tiles[1]);
		const TerrainType& terrain2 = Li::Application::Get<ArticleApp>().GetTerrainData().GetTerrainPrototype(chunk.Tiles[2]);
		const TerrainType& terrain3 = Li::Application::Get<ArticleApp>().GetTerrainData().GetTerrainPrototype(chunk.Tiles[3]);
