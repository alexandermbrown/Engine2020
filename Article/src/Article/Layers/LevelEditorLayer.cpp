	...
	: Layer("LevelEditor"), m_DockspaceOpen(true), m_TitleHasAsterisk(false), m_Viewport(&m_Settings), m_TerrainEditingPanel(&m_Settings)
{
	...
	SDL_SetWindowTitle(Li::Application::Get().GetWindow().GetWindow(), "Article Level Editor");
}

LevelEditorLayer::~LevelEditorLayer()
{
	SDL_SetWindowTitle(Li::Application::Get().GetWindow().GetWindow(), "Article");
}
...
void LevelEditorLayer::OnUpdate(Li::Duration::us dt)
{
	...
	{
		SDL_SetWindowTitle(Li::Application::Get().GetWindow().GetWindow(), (m_TerrainPath.filename().string() + "* - Article Level Editor").c_str());
...
void LevelEditorLayer::OnImGuiRender()
{
	...
	if (m_NewTerrainModal.IsDone())
	{
		...
		{
			SDL_SetWindowTitle(Li::Application::Get().GetWindow().GetWindow(), "Article Level Editor");
			SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Article Level Editor", "Failed to open new terrain file.", Li::Application::Get().GetWindow().GetWindow());
		}
	}
}
...
void LevelEditorLayer::FileOpenDialog()
{
	...
	if (result == NFD_OKAY)
	{
		...
		{
			SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Article Level Editor", "Failed to open terrain file.", Li::Application::Get().GetWindow().GetWindow());
		}
	}
